  ...
  server.get('/author', (req, res) => {
    ...
    server.post('/register', (req, res) => {
      ...
      pool.query(sql, [uname], (error, results) => {
        ...
        if (count == 0) {
          // 将用户的相关信息插入到数据表
          sql = 'INSERT np_user(user_id,upwd) VALUES(?,MD5(?))';
          pool.query(sql, [user_id, upwd], (error, results) => {
            if (error) throw error;
    ...
    server.post('/login', (req, res) => {
      ...
      let upwd = req.body.upwd;
      // SQL语句
      let sql = 'SELECT user_id,phone,avatar FROM np_user WHERE uname=? AND pwd=MD5(?)';
